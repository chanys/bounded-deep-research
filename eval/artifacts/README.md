...
Only the small, human-reviewed artifacts are committed to git: the final query sets (`queryset_*.jsonl`), the readable corpus inventory (`inventory_code4AI.md`), the methods record (`generation_conditions.md`), the longitudinal candidate shortlist (`longitudinal_shortlist.md`), the longitudinal grounding-input manifest (`longitudinal_grounding_ids.json`), the edited longitudinal gold worksheet (`longitudinal_claim_vs_grounder.md`), and the Task 7 handoff set (`{factual,comparative,longitudinal}_claimslice_handoff.md`, `SLICES.md`, `grounding_prediction_note.md`).
...
- **`longitudinal_claim_vs_grounder.md`** - the **longitudinal gold worksheet** (37 candidates), and the one worksheet committed to git (script-rendered then hand-edited). Per candidate it shows the question, its trajectory arc, and each dated milestone's source chunk(s) tagged `[grounded]` / `[not re-found]`, plus other grounder chunks. This is the working surface the `gold-editing-longitudinal` skill (in the code repo under `.claude/skills/`) edits: each arc is turned into a flat list of gradeable **nuggets** (a stance plus a time window), every milestone tagged `supports: nK`, one shift nugget per question (or its no-change mirror when the view stayed constant), with voice, orphan, granularity, and coverage fixes applied and any unresolved calls handed to a human as decidable flags. Longitudinal is out of scored scope (D75), so this is exploratory and method-development, committed as a small reviewed artifact (gold-v0.1). The editing session that produced it is recorded in `phase4_longitudinal_gold_edit_build_log.md` (notes repo).
