...
Runs the full ReAct agent on a single query so the wiring can be verified in the
Langfuse trace:
  - system message contains "Critical Failure Policy" (load_recipe is active)
  - at least one search_transcripts call appears in the tool calls
  - recipe_version in the trace metadata matches the active recipe frontmatter

The run may take 30-60s (multiple searches, then submit). The trace is flushed to
Langfuse on exit; open the Langfuse console to inspect it.
